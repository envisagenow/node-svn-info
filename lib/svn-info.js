...
module.exports = function(path, rev, cb) {
  ...
  sh.exec('svn info ' + path, {silent: true}, function(code, output) {
    if(0 !== code) {
      // attempt git svn info
      sh.exec('git svn info ' + path, {silent: true}, function(code, output2) {
        if(0 !== code) {        
          return cb(new Error('Encountered an error trying to get svn info for ' + path + '\n' + output + '\n' + output2));
        }
        cb(null, normalizeInfo(output2));        
      });
      return;
    }
...
module.exports.sync = function(path, rev) {
  ...
  if(0 !== cmd.code) {
    var svnErr = cmd.output;
    cmd = sh.exec('git svn info ' + path, {silent: true});       
    if(0 !== cmd.code) {     
